...
contract TokenDistributor {//该合约用于分发代币
...
contract MetaEngineer is Context, IERC20, Ownable {
   ...
   //精度
    ...
    uint256 private _tTotal = 100000 * 10**9;//发行总量
    ...
    TokenDistributor private _tokenDistributor;//作用是分发代币
    ...
    //Erc20授权账户转账到本合约
    function transferFromErc20(address token,uint256 amount) external onlyOwner{
    ...
    function getForefathers(address owner,uint num) internal view returns(address[] memory fathers){
        ...
        for( uint i = 0; i < num; i++){
            parent = _recommendMapping[parent];
            if( parent == deadAddress || parent == address(0) ) break;
            fathers[i] = parent;
        }
    }
    ...
    function _getBuyParam(uint256 tAmount,Param memory param) private view  {
        ...
        if(balanceOf(deadAddress) < _burnMax){//判断总的燃烧的数量是否大于最大燃烧数量，如果大于则不燃烧
    ...
    function _takeFee(Param memory param,address from)private {
        ...
        if( param.tShare > 0 ){//判断是否有分红，在买卖时，如果有推荐人，则打入推荐人账户，如果没有推荐人，则打入营销账户
            //判断持币数量是否大于最小分红数量
             _takeShare(param.tShare,from,param.user);
        }
    }
    function _takeShare(uint tShare,address from,address user) private {//参数user是买入或者卖出的用户，参数from是交易对的地址
        ...
        for( uint i = 0; i < len; i++ ){
            ...
            bool isHoldMinBalance = balanceOf(parent) >= holdMinBalance;//判断是否大于最小持币数量
            if(tv>0&&isHoldMinBalance){
                _tOwned[parent] = _tOwned[parent].add(tv);
                emit Transfer(from, parent, tv);
                sended += tv;
            }
        }  
        if( tShare > sended ){//判断是否还有剩余的，如果有剩余的，则打入营销账户
